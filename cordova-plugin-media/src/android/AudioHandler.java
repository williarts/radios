...
public class AudioHandler extends CordovaPlugin {
    ...
    ArrayList<AudioPlayer> pausedForPhone; // Audio Player.getInstance().getPlayers() that were paused when phone call came in
    ArrayList<AudioPlayer> pausedForFocus; // Audio Player.getInstance().getPlayers() that were paused when focus was lost
    ...
    /**
     * Stop all audio Player.getInstance().getPlayers() and recorders.
     */
    public void onDestroy() {
        if (!Player.getInstance().getPlayers().isEmpty()) {
            onLastPlayerReleased();
        }
        for (AudioPlayer audio : Player.getInstance().getPlayers().values()) {
            audio.destroy();
        }
        Player.getInstance().getPlayers().clear();
    }

    /**
     * Stop all audio Player.getInstance().getPlayers() and recorders on navigate.
     */
    @Override
    public void onReset() {
        onDestroy();
    ...
    public Object onMessage(String id, Object data) {
        ...
        if (id.equals("telephone")) {
            ...
            if ("ringing".equals(data) || "offhook".equals(data)) {
                ...
                // Get all audio Player.getInstance().getPlayers() and pause them
                for (AudioPlayer audio : Player.getInstance().getPlayers().values()) {
            ...
            // If phone idle, then resume playing those Player.getInstance().getPlayers() we paused
    ...
    private AudioPlayer getOrCreatePlayer(String id, String file) {
        AudioPlayer ret = Player.getInstance().getPlayers().get(file);
        if (ret == null) {
            if (Player.getInstance().getPlayers().isEmpty()) {
                onFirstPlayerCreated();
            }
            ret = new AudioPlayer(this, id, file);
            Player.getInstance().getPlayers().put(file, ret);
        }
        return ret;
    }
    ...
    private boolean release(String id) {
        AudioPlayer audio = Player.getInstance().getPlayers().remove(id);
        if (audio == null) {
            return false;
        }
        if (Player.getInstance().getPlayers().isEmpty()) {
    ...
    public void stopRecordingAudio(String id, boolean stop) {
        AudioPlayer audio = Player.getInstance().getPlayers().get(id);
        if (audio != null) {
